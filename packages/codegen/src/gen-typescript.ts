 ...
 *
 * The generated code uses BinaryWriter/BinaryReader from @bufbuild/protobuf
...
function generateMessageClass(
  ...
  lines.push('    const r = new BinaryReader(data);');
  lines.push(`    const msg = new ${msg.name}();`);
  lines.push('    while (r.pos < r.len) {');
  lines.push('      const [fieldNumber, wireType] = r.tag();');
...
function generateClientMethod(m: MethodDef): string {
  ...
  if (!m.clientStreaming && !m.serverStreaming) {
    ...
    lines.push('      options,');
    lines.push('    );');
    lines.push(`    return ${respType}.decode(result);`);
    lines.push('  }');
