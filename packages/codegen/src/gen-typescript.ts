...
export function generateClient(proto: ProtoFile): string {
  ...
  lines.push('// -----------------------------------------------------------------');
  lines.push('');
  lines.push("import { RpcClient, type CallOptions, type Subscription } from '@rpc-bridge/core';");
...
function generateClientMethod(m: MethodDef): string {
  ...
  if (!m.clientStreaming && !m.serverStreaming) {
    ...
    lines.push('  }');
  } else if (!m.clientStreaming && m.serverStreaming && m.startupErrorType) {
    // --- Server streaming with startup error ---
    const errType = m.startupErrorType;
    const hasErrCodec = hasJsonCodec(errType);
    lines.push(`  /** Server-streaming RPC: ${m.name} (startup_error: ${errType}) */`);
    lines.push(`  async ${tsName}(request: ${reqType}, options?: CallOptions): Promise<Subscription<${respType}, ${errType}>> {`);
    if (hasReqCodec) {
      lines.push(`    const data = this.json ? ${reqType}JSON.encode(request) : request;`);
    } else {
      lines.push(`    const data = request;`);
    }
    lines.push(`    const result = await this.client.serverStreamWithStartupError(\`${methodPath}\`, data, options);`);
    lines.push(`    if (!result.ok) {`);
    if (hasErrCodec) {
      lines.push(`      return { ok: false, error: (this.json ? ${errType}JSON.decode(result.error as Record<string, unknown>) : result.error) as ${errType} };`);
    } else {
      lines.push(`      return { ok: false, error: result.error as ${errType} };`);
    }
    lines.push(`    }`);
    // Wrap events to decode response type
    lines.push(`    const self = this;`);
    lines.push(`    const events = (async function* () {`);
    lines.push(`      for await (const item of result.events) {`);
    if (hasRespCodec) {
      lines.push(`        yield (self.json ? ${respType}JSON.decode(item as Record<string, unknown>) : item) as ${respType};`);
    } else {
      lines.push(`        yield item as ${respType};`);
    }
    lines.push(`      }`);
    lines.push(`    })();`);
    lines.push(`    return { ok: true, events };`);
    lines.push('  }');
...
function generateServerInterface(svc: ServiceDef): string {
  ...
  for (const m of svc.methods) {
    ...
    if (!m.clientStreaming && !m.serverStreaming) {
      ...
      if (m.startupErrorType) {
        lines.push(`  /** Server-streaming: ${m.name} (startup_error: ${m.startupErrorType}) */`);
      } else {
...
function generateServerDispatcher(pkg: string, svc: ServiceDef): string {
  ...
  for (const m of svc.methods) {
    ...
    if (!m.clientStreaming && !m.serverStreaming) {
      ...
      // --- Server streaming ---
      const errType = m.startupErrorType;
      const hasErrCodec = errType ? hasJsonCodec(errType) : false;
      lines.push(`  methods['${m.name}'] = {`);
      lines.push(`    type: ${methodTypeStr},`);
      ...
      if (hasRespCodec || (errType && hasErrCodec)) {
        lines.push('      return (async function* () {');
        if (errType && hasErrCodec) {
          lines.push('        try {');
          lines.push(`          for await (const resp of responses) yield json ? ${respType}JSON.encode(resp) : resp;`);
          lines.push('        } catch (err) {');
          lines.push(`          if (json && err instanceof StartupError) throw new StartupError(${errType}JSON.encode(err.details as ${errType}), err.message);`);
...
function collectClientImports(proto: ProtoFile): { types: Set<string>; jsonCodecs: Set<string> } {
  ...
  for (const svc of proto.services) {
    for (const m of svc.methods) {
      ...
      if (m.startupErrorType) {
        types.add(m.startupErrorType);
        if (hasJsonCodec(m.startupErrorType)) jsonCodecs.add(`${m.startupErrorType}JSON`);
      }
    }
  }
